...
func TestCanForceNow(t *testing.T) {
  ...
  ForceNow(expected)
  actual := Now()
  if actual.Equal(expected) == false { t.Errorf("Now should equal %q but equals %q", expected, actual) }
}
...
func assertNowIsNow(t *testing.T) {
  ...
  if actual.Before(start) { t.Errorf("time should not be before %q", start) }
  if actual.After(end) { t.Errorf("time should not be after %q", end) }
}
